  ...
  if (!name || name === 'Grand Total') {
    if (name === 'Grand Total') {
      gpVillageSummary.push({
        ...
        cultivableLandBigha: Number(row[4]) || 0,
        leaseLandBigha: Number(row[5]) || 0,
        totalCultivatedBigha: Number(row[6]) || 0,
...
const householdProfiles = farmerRows.map((row) => {
  ...
  return {
    ...
    // Land
    cultivableLandBigha: Number(row[53]) || 0,
    leaseLandBigha: Number(row[54]) || 0,
    totalCultivatedBigha: Number(row[55]) || 0,
    // Farming economics
...
const metricLabels = {
  ...
  '6.1': 'AAS Membership',
  '8': 'Total Cultivable Land (Bigha)',
  '8.1': 'Avg Land per Family (Bigha)',
  ...
  '10.3': 'Avg Cost per Bigha (INR)',
  '10.4': 'Avg Income per Bigha (INR)',
  '10.5': 'Total Fertilizer Use (Kgs)',
  '10.6': 'Avg Fertilizer per Bigha (Kgs)',
  '10.7': 'Total Farmers',
...
const aggregateMetrics = {
  ...
  // Land
  totalCultivableLandBigha: getIndicator('8')?.value || 0,
  avgLandPerFamilyBigha: getIndicator('8.1')?.value || 0,
  ...
  avgCostPerBighaRs: getIndicator('10.3')?.value || 0,
  avgIncomePerBighaRs: getIndicator('10.4')?.value || 0,
